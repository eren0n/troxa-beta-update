...
class GenerationJob(models.Model):
    ...
    # Which surface started this job. Only consulted for the gallery's
    # "generated by" label when created_by is null, which happens when the job
    # came from a Slack channel — there is no Slack-to-Troxa user mapping, so
    # those show up as "Slack" rather than as somebody who didn't do it.
    origin = models.CharField(max_length=20, default='dashboard')
    # Slack channel the request came from, so the finished images report back
    # where they were asked for rather than wherever auto-posting happens to
    # be switched on.
    origin_channel = models.CharField(max_length=50, blank=True, default='')
