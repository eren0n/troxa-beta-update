...
class GeneratedCreativeSerializer(serializers.ModelSerializer):
    ...
    def get_created_by_name(self, obj):
        ...
        # A job started from a Slack channel has no Troxa user behind it (no
        # Slack-to-Troxa mapping exists), so name the surface instead of
        # leaving the gallery's "generated by" blank.
        if obj.job and obj.job.origin == 'slack':
            return 'Slack'
        return None
