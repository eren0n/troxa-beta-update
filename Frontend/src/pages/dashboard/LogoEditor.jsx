...
export default function LogoEditor() {
  ...
  // ── Init Fabric.js after job loads ──
  useEffect(() => {
    if (!job || !canvasRef.current) return;
    const { fabric } = window;
    if (!fabric) { setError('Fabric.js failed to load. Check network.'); return; }
    ...
    return () => { canvas.dispose(); window.removeEventListener('keydown', onKey); };
  }, [job]);
      ...
      <div className="min-h-screen bg-[#05070a] text-white">
        <div className="max-w-4xl mx-auto py-10 px-6 space-y-6">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/dashboard/creatives')}
    ...
    <div className="flex flex-col bg-[#05070a] text-white" style={{ height: '100dvh' }}>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-[#060810] overflow-hidden max-lg:min-h-[35vh]">
          ...
          {creatives.length > 1 && (
            <div className="shrink-0 h-[72px] border-t border-white/5 bg-[#0a0d14] flex items-center gap-2.5 px-4 overflow-x-auto">
              {creatives.map((c, i) => (
                <button key={c.id} onClick={() => handleSwitchImage(i)}
                  className={`shrink-0 h-12 w-12 rounded-lg overflow-hidden border-2 transition-all ${
                    i === currentIdx ? 'border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.4)]' : 'border-white/10 hover:border-white/30'
                  }`}>
                  <CreativeImg creativeId={c.id} className="w-full h-full object-cover" alt={`Image ${i + 1}`} />
                </button>
              ))}
            </div>
          )}
