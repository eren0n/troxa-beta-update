...
import {
  ...
  Download, Loader2, ExternalLink, Calendar, Sparkles,
  Smartphone, X, Copy, CheckCircle,
} from 'lucide-react';
        ...
        {dangerConfirm && (
          ...
          >
            <motion.div
              ...
              className="bg-[#10141d] border border-red-500/30 rounded-3xl p-8 max-w-md w-full space-y-6"
        ...
        {twoFAModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-[#10141d] border border-white/10 rounded-3xl p-8 max-w-md w-full space-y-6"
            >
              {twoFAModal === 'setup' && (
                <>
                  {twoFAStep === 'done' ? (
            ...
            >
              ...
              {activeTab === 'account' && (
                <div className="space-y-6">
                  <div className="p-8 bg-[#10141d] border border-white/5 rounded-3xl space-y-6">
                    <div className="flex items-center justify-between">
              ...
              {activeTab === 'notifications' && (
                <div className="p-8 bg-[#10141d] border border-white/5 rounded-3xl space-y-8">
              ...
              {activeTab === 'appearance' && (
                <div className="space-y-6">
                  <div className="p-8 bg-[#10141d] border border-white/5 rounded-3xl space-y-8">
                    <h3 className="text-sm font-extrabold uppercase tracking-widest text-white">Appearance</h3>

                    <div className="space-y-3">
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Theme</label>
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          { id: 'dark', label: 'Dark (Default)', preview: 'bg-zinc-950 border-zinc-800' },
                          { id: 'darker', label: 'Pure Black', preview: 'bg-black border-white/5' },
                          { id: 'midnight', label: 'Midnight Blue', preview: 'bg-blue-950 border-blue-900' },
                        ].map((theme) => (
                          <button
                            key={theme.id}
                            onClick={() => setAppearance(prev => ({ ...prev, theme: theme.id }))}
                            className={`p-4 rounded-2xl border transition-all text-left ${appearance.theme === theme.id ? 'border-blue-500 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'border-white/5 hover:border-white/10'}`}
                          >
                            <div className={`w-full h-12 rounded-lg border ${theme.preview} mb-3`} />
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-3">
                        {accentColors.map((c) => (
                          <button
                            key={c.id}
                            onClick={() => setAppearance(prev => ({ ...prev, accentColor: c.id }))}
                            className={`w-8 h-8 rounded-full border-2 transition-all ${appearance.accentColor === c.id ? 'border-white scale-110 shadow-lg' : 'border-transparent'}`}
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">UI Density</label>
                      <div className="grid grid-cols-3 gap-3">
                        {['compact', 'comfortable', 'spacious'].map((d) => (
                          <button
                            key={d}
                            onClick={() => setAppearance(prev => ({ ...prev, density: d }))}
                            className={`py-3 rounded-xl border text-xs font-bold uppercase tracking-wider transition-all ${appearance.density === d ? 'border-blue-500 bg-blue-500/10 text-white' : 'border-white/5 text-gray-400 hover:text-white hover:border-white/10'}`}
                          >
                            {d}
                          </button>
                        ))}
                    ...
                    <div className="space-y-4 border-t border-white/5 pt-6">
                      {[
                        { key: 'animationsEnabled', label: 'Enable UI Animations', desc: 'Spring-physics motion effects and transitions throughout the app' },
                        { key: 'sidebarCompact', label: 'Start with Compact Sidebar', desc: 'Always open the dashboard with the sidebar in icon-only mode' },
                      ].map((item) => (
                        <div key={item.key} className="flex items-center justify-between gap-4 p-4 rounded-2xl hover:bg-white/2 transition-colors">
                          <div>
                            <p className="text-sm font-semibold text-white">{item.label}</p>
                          ...
                          <Toggle
                            ...
                            onToggle={() => setAppearance(prev => ({ ...prev, [item.key]: !prev[item.key] }))}
                          />
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                      <button
                        ...
                        className="px-4 py-2.5 bg-zinc-900 border border-white/5 text-gray-400 rounded-xl text-sm font-bold transition-colors flex items-center gap-2 hover:text-white"
                      >
                        <RefreshCw className="w-4 h-4" /> Reset Defaults
                      </button>
                      <button
                        onClick={handleSaveAppearance}
                        className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-600/20 transition-colors"
                      >
                        Save Appearance
                      </button>
                    </div>
              ...
              {activeTab === 'billing' && (
                <div className="space-y-6">
                  {/* Plan card */}
                  <div className="p-8 bg-linear-to-br from-blue-900/10 to-[#10141d] border border-blue-500/10 rounded-3xl space-y-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-[10px] bg-blue-500/10 border border-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full font-black uppercase tracking-wider">{planName}</span>
                        <h3 className="text-2xl font-black text-white mt-2">
                          {creditTotal} <span className="text-gray-500 text-base font-normal">total credits</span>
                        </h3>
                        <p className="text-xs text-gray-500 mt-1">{creditBalance} remaining · {creditUsed} used</p>
                      </div>
                      <Link
                        to="/dashboard/billing"
                        className="px-4 py-2 bg-zinc-900 border border-white/5 text-gray-300 rounded-xl text-xs font-bold hover:text-white transition-colors flex items-center gap-2"
                      >
                        <ChevronRight className="w-4 h-4 text-blue-400" /> Manage Plan
                      </Link>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500">Usage</span>
                        <span className="text-white font-bold">{creditPct}%</span>
                      </div>
                      <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                        <motion.div
                          ...
                          className={`h-full rounded-full ${creditPct > 80 ? 'bg-linear-to-r from-red-600 to-red-400' : 'bg-linear-to-r from-blue-600 to-blue-400'}`}
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/5">
                      {[
                        { label: 'Credits Used', value: creditUsed.toLocaleString() },
                        { label: 'Credits Left', value: creditBalance.toLocaleString() },
                        { label: 'Total Credits', value: creditTotal.toLocaleString() },
                      ].map((item) => (
                        <div key={item.label}>
                  ...
                  {/* Transaction history */}
                  <div className="p-8 bg-[#10141d] border border-white/5 rounded-3xl space-y-4">
                    <h3 className="text-sm font-extrabold uppercase tracking-widest text-white flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-blue-500" /> Credit History
                    </h3>
                    {txLoading ? (
                      <div className="flex justify-center py-8">
                      ...
                      <p className="text-xs text-gray-600 italic py-4">No transactions yet.</p>
                    ) : (
                      <div className="space-y-3">
                        {transactions.map((tx) => (
                          <div key={tx.id} className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-2xl">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="text-sm font-bold text-white">{tx.description || (tx.transaction_type === 'credit' ? 'Credits added' : 'Credits used')}</p>
                                <p className="text-[10px] text-gray-500">{formatTxDate(tx.created_at)}</p>
                              </div>
                            </div>
                            <span className={`text-sm font-black ${tx.transaction_type === 'credit' ? 'text-emerald-400' : 'text-red-400'}`}>
