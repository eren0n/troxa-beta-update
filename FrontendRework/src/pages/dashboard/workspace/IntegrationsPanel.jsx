...
import { Key, Check, Trash2, Copy, ExternalLink, Hash, Loader2, HardDrive, FolderOpen, Download, Search, X, RefreshCw, ToggleLeft, ToggleRight, ImageIcon, Plus, ChevronDown } from 'lucide-react';
...
function ChannelRow({ sc, headers, onUpdate, onDelete }) {
  ...
  const patch = async (body, optimistic) => {
    if (optimistic) onUpdate({ ...sc, ...optimistic });
    try {
      const res = await fetch(`/api/slack/channels/${sc.id}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', ...headers },
  ...
  const toggleType = (type) => {
    ...
    // also remove from auto_post_types if disabling
    const auto = (sc.auto_post_types || []).filter(t => next.includes(t));
    patch({ content_types: next, auto_post_types: auto }, { content_types: next, auto_post_types: auto });
  };

  const toggleAutoPost = (type) => {
    const current = sc.auto_post_types || [];
    const next = current.includes(type) ? current.filter(t => t !== type) : [...current, type];
    patch({ auto_post_types: next }, { auto_post_types: next });
  };
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(CONTENT_TYPE_LABELS).map(([key, lbl]) => {
            ...
            const autoOn = (sc.auto_post_types || []).includes(key);
            return (
              <div key={key} className="flex items-center gap-0 rounded-xl overflow-hidden border border-white/8">
                {/* Content type toggle */}
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] font-bold transition-all ${
                    active
                      ...
                      : 'bg-white/3 text-gray-600 hover:text-gray-400'
                  }`}
                >
                  {lbl}
                </button>
                {/* Auto-post switch — only when type is active */}
                <AnimatePresence>
                  {active && (
                    <motion.button
                      key="auto"
                      ...
                      className={`overflow-hidden flex items-center gap-1 pl-2 pr-2.5 py-1.5 text-[10px] font-black uppercase tracking-wider border-l transition-all ${
                        autoOn
                          ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
                          : 'bg-white/3 border-white/8 text-gray-600 hover:text-gray-400'
                      }`}
                      title={autoOn ? 'Auto-post ON — disable for manual only' : 'Auto-post OFF — click to enable'}
                    >
                      {/* mini toggle pill */}
                      <span className={`relative inline-flex h-3 w-5 shrink-0 rounded-full transition-colors ${autoOn ? 'bg-emerald-500' : 'bg-white/15'}`}>
                        <span className={`absolute top-0.5 h-2 w-2 rounded-full bg-white shadow transition-transform ${autoOn ? 'translate-x-2.5' : 'translate-x-0.5'}`} />
                      </span>
                      Auto
                    </motion.button>
                  )}
        ...
        <p className="text-[9px] text-gray-700 mt-2">
          Auto = posts automatically when content is generated. Manual = only via Post to Slack button.
        </p>
      </div>
    </div>
  );
...
function IntegrationsPage() {
  ...
  return (
    ...
    >
      <p className="text-slate-500 text-sm">Connect external services and configure webhooks</p>
