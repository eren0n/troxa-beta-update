                ...
                <div className="p-5 space-y-5 flex-1">
                  ...
                  {c && (
                    <div className="space-y-2.5">
                      {(() => {
                        // An upload wasn't generated by anyone — credit the uploader.
                        const uploaded = c.source === 'uploaded';
                        const by = uploaded
                          ? (c.uploaded_by_name || c.generated_by_name || c.created_by_name)
                          : (c.generated_by_name || c.created_by_name);
                        return by && (
                          <div>
                            <p className="text-[10px] font-black text-(--text-muted) uppercase tracking-widest mb-0.5">{uploaded ? 'Uploaded by' : 'Generated by'}</p>
                            <p className="text-xs text-(--text-primary)">{by}</p>
                          </div>
                        );
                      })()}
                      {c.model_name && (
