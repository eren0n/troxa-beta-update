...
import { Settings, LogOut, User, ChevronDown, Sparkles, Shield } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useAuth } from '../../contexts/AuthContext';

export const ProfileDropdown = ({ variant = 'header', collapsed = false }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={
          isSidebar
            ? `flex items-center gap-2.5 rounded-xl transition-all duration-200 group cursor-pointer w-full hover:bg-white/5 ${collapsed ? 'px-2 py-2.5 justify-center' : 'px-3 py-2.5'}`
            : 'flex items-center gap-2.5 pl-2 group cursor-pointer'
        }
      >
        <AvatarEl size="sm" />
        {(!isSidebar || !collapsed) && (
          <div className={isSidebar ? 'flex-1 text-left min-w-0' : 'hidden lg:block text-left'}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={
              isSidebar
                ? 'absolute bottom-full left-0 mb-3 w-64 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden'
                : 'absolute top-full right-0 mt-3 w-64 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden'
            }
          >
            {/* User Info */}
            <div className="p-4 border-b border-white/5" style={{ background: 'linear-gradient(to bottom right, var(--accent-muted), transparent)' }}>
              ...
              <div className="mt-3">
                ...
                <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${credits ? Math.min(100, (balance / (credits.total || balance || 1)) * 100) : 0}%`,
